...
<div>
    @foreach($data as $row)
        <div>{{ $row->hashtag->id }} : {{ $row->hashtag->name }} : {{ $row->total_view_count }}</div>
        <div>Total View Count: {{ $row->total_view_count }}</div>
        <div>Last Update Datetime: {{ $row->last_update_datetime }}</div>
